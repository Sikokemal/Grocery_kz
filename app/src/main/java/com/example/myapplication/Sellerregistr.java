...
public class Sellerregistr extends AppCompatActivity {
    ...
    private EditText editTextKaspiNumber;
    ...
    private void loginUser() {
        ...
        String kaspi = editText.getText().toString().trim();
        ...
        if (email.isEmpty() || password.isEmpty()) {
            Toast.makeText(Sellerregistr.this, "Please enter email and password", Toast.LENGTH_SHORT).show();
            hideLoading();
            return;
        }
        ...
        auth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        ...
                        String userType = "продавец";
                        uploadImageToStorage(uid, email, name, userType, password, address, phone,kaspi);
                        ...
                        startActivity(new Intent(Sellerregistr.this, MainActivity.class));
                        finish();
                    } else {
                        Toast.makeText(Sellerregistr.this, "Registration failed: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                        hideLoading();
                    }
                });
    }
        ...
        if (user != null) {
            user.sendEmailVerification()
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            Toast.makeText(Sellerregistr.this, "Verification email sent to " + user.getEmail(), Toast.LENGTH_SHORT).show();
                            // После успешной отправки кода верификации можно выполнить какие-либо дополнительные действия, если необходимо
                        } else {
                            Toast.makeText(Sellerregistr.this, "Failed to send verification email.", Toast.LENGTH_SHORT).show();
                        }
                    });
        }
    }
    ...
    private String getUserType() {
        ...
        switch (selectedRadioButtonId) {
            ...
            case R.id.radioButtonSeller:
                return "seller";
            default:
                return "buyer"; // По умолчанию
        }
    }

    private void uploadImageToStorage(String uid, String email,String name, String userType, String password, String address, String phone,String kaspi) {
        if (imageUri != null) {
            ...
            imageRef.putFile(imageUri)
                    .addOnSuccessListener(taskSnapshot -> {
                        imageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                            ...
                            saveUserToDatabase(uid, email,name, userType, password, address, phone,kaspi, imageUrl);
                        });
                    })
            ...
            saveUserToDatabase(uid, email,name, userType, password, address, phone,kaspi, null);

        }
    }


    private void saveUserToDatabase(String uid, String email,String name, String userType, String password, String address, String phone,String kaspi, String imageUrl) {
        DatabaseReference userReference = FirebaseDatabase.getInstance().getReference().child("users").child(uid);

        users user = new users(uid, email,name, userType, password, address, phone,kaspi, imageUrl);
